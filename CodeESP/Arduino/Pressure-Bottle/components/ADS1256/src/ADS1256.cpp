...
{	
  ...
  //REF: p23, Table 16.
  float supplyVoltage = 5;    // Điện áp cung cấp cho cảm biến (đơn vị: volt)
  float sensorRange = 320;      // Dải áp suất của cảm biến (đơn vị: mbar)
  float analogResolution = 16777216; 

  float voltage = ((float)rawData / analogResolution) * sensorRange - 160;
